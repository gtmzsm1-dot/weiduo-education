...
## 协作纪律:spec 边界与"贴心改动"陷阱(2026-05-10 补充)

### 教训背景

阶段 8 实施时,WorkBuddy 在 doCloudLogin 失败路径加了 doLocalLogin 回退——超出 spec 范围,且未主动汇报。事后审查移除。

### 反模式识别

以下是工程协作里典型的"贴心改动"陷阱,任何 Agent 都应警惕:

1. **"用户测试时不方便,我顺手加个兜底"**——把"开发者自测便利"和"真实用户体验"混淆,牺牲架构纯度
2. **"多一层容错总比没有好"**——容错只在"对真实用户有价值"时才是容错,否则是技术债
3. **"我用 commit message 描述了,等于汇报了"**——commit message 是历史记录,不是主动汇报。**汇报必须在和 owner/审查者的沟通文本里明示**
4. **"反正测试通过了"**——测试通过 ≠ spec 遵守。spec 是契约,边界不是测试结果决定的

### 强制规则

任何超出 spec 的改动,**必须满足以下三条之一**才能保留:

1. **事先报备**:在执行前向 owner 说明"我建议增加 X 设计,理由 Y",等批准
2. **事后明示**:在汇报里**单独一行**说"⚠️ 我加了 spec 之外的 X 改动,理由 Y,请审查"
3. **owner 主动批准过类似扩展**:在历史对话或 docs 里有明文授权

不满足任一条 = 违反协作纪律,即使代码正确也必须 revert + 复盘。

### 自查口诀

提交前问自己三个问题:

- **这个改动 spec 里写了吗?**(如未写 → 是扩展)
- **如果是扩展,我汇报了吗?**(如未明示 → 必须补)
- **这个扩展对真实用户有价值,还是只对开发者方便?**(如只为方便 → 大概率应该移除)

## 核心字段改造的"全场景扫描"要求(2026-05-10 补充)

### 教训背景

阶段 2.3 改造 archiveCode(从顺序数字到高熵随机),但当时只关注了**生成入口**(genCode)和**查询入口**(doLogin)。**遗漏了消费方** buildQRList——这个函数独立用 archiveCode 生成二维码,migrate 后未被刷新,导致二维码内容过期。

直到阶段 8 真机测试 Step C 才暴露此 bug。

### 必须遵守

改造任何**会被多处消费的核心字段**时,**修改之前**必须做"全场景扫描":

```bash
# 模板:改造 X 字段前先 grep 所有引用
grep -n "\.X\|'X'|\"X\"" index.html
```
...
- 如果是读取,会受改造影响吗?
- 如果会,是否需要同步改造?
- 如果不需要立即改造,是否需要在改造后触发某种刷新?

### 反模式

❌ "我只改了 genCode 和 doLogin,业务正常工作了" → 实际可能有未被发现的消费方
❌ "看起来没影响,先上线再说" → 等 bug 暴露时,可能已经有真实用户受影响
✅ 改造前 grep → 列清单 → 逐个评估 → 然后才动手
